...
CMD="cd ${WORKDIR} && python3 scripts/stage3_cli.py --config ${CONFIG_PATH} --override ${OVERRIDE_PATH} train --model ${MODEL_NAME} --dataset ${DATASET_NAME} --run-group ${RUN_GROUP} --run-name ${RUN_NAME} --resume"
