...
from .utils import ensure_dir, load_json, save_json, set_seed, slugify, timestamp
...
def evaluate_existing_run(run_dir: str | Path, config: dict[str, Any], smoke_mode: bool = False) -> dict[str, Any]:
    ...
    stored_context = load_json(run_path / "context.json")
    ...
    primary_method = str(config["threshold"]["primary_method"])
    primary_metrics = next((item for item in threshold_results if item["threshold_method"] == primary_method), threshold_results[0])
    save_json(run_path / "scoring_reference.json", scorer.serialize_reference())
    write_run_outputs(
        run_dir=run_path,
...
def evaluate_checkpoint_to_run(
    config: dict[str, Any],
    ...
    stored_context = load_json(source_path / "context.json")
    dataset_name = dataset_name or stored_context["dataset_name"]
    model_name = model_name or stored_context["model_name"]
    checkpoint_path = source_path / "checkpoints" / checkpoint_name
    ...
    context = {
        ...
        "scoring_method": model_cfg.get("scoring", {}).get("method", "reconstruction"),
        "primary_threshold_method": primary_method,
        "training_summary": stored_context.get("training_summary", {}),
        "dataset_metadata": bundle.metadata,
        "source_run_dir": str(source_path),
